...
def create_random_name():
    return random.choice(string.ascii_uppercase) + ''.join(random.choices(string.ascii_lowercase, k=random.randint(2,10)))

def create_aspect(rank):
    points = 7+rank
    max_points = points
...
def test_aspect(rank):
    attrs = create_aspect(rank)
